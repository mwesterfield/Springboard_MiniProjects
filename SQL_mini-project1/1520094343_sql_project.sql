...
SELECT firstname,
  ...
  JOIN ( SELECT MAX(joindate) AS latestjoindate
           FROM country_club.Members
       ) sub
    ON joindate = sub.latestjoindate
...
SELECT CONCAT(members.surname, ', ', members.firstname) AS fullname,
  ...
  JOIN (
    ...
    ON members.memid = tennis_bookings.memid
 WHERE members.surname NOT LIKE 'GUEST%'
 GROUP BY fullname, tennis_bookings.name
 ORDER BY fullname
...
SELECT facilities.name AS facility_name,
       CASE WHEN members.surname = 'GUEST' THEN 'GUEST'
            ELSE CONCAT(members.surname, ', ', members.firstname) END AS full_name,
       CASE WHEN bookings.memid = 0 THEN bookings.slots * facilities.guestcost
            ELSE bookings.slots * facilities.membercost END AS totalcost
  FROM country_club.Bookings bookings
  JOIN country_club.Members members
    ON bookings.memid = members.memid
  JOIN country_club.Facilities facilities
    ON bookings.facid = facilities.facid
 WHERE (LEFT(bookings.starttime,10) LIKE '2012-09-14%')
HAVING totalcost > 30
...
SELECT sub.facility_name,
       SUM(sub.totalcost) AS totalrevenue
  FROM (SELECT facilities.name AS facility_name,
               CASE WHEN bookings.memid = 0 THEN bookings.slots * facilities.guestcost
                    ELSE bookings.slots * facilities.membercost END AS totalcost
          FROM country_club.Bookings bookings
